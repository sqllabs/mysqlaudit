...
func (s *testSuite) TestPointGet(c *C) {
	...
	tk.MustExec("create table point1 (id int primary key, c int, d varchar(10), unique c_d (c, d))")
	tk.MustExec("insert point1 values (1, 1, 'a')")
	tk.MustExec("insert point1 values (2, 2, 'b')")
	tk.MustQuery("select * from point1 where id = 1 and c = 0").Check(testkit.Rows())
	tk.MustQuery("select * from point1 where id < 0 and c = 1 and d = 'b'").Check(testkit.Rows())
	result, err := tk.Exec("select id as ident from point1 where id = 1")
	c.Assert(err, IsNil)
